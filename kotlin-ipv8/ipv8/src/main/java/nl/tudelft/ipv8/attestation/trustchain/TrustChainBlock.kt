...
class TrustChainBlock(
    ...
    override fun toString(): String {
        return "TrustChainBlock(type='$type', rawTransaction=${rawTransaction.contentToString()}, publicKey=${publicKey.contentToString()}, sequenceNumber=$sequenceNumber, linkPublicKey=${linkPublicKey.contentToString()}, linkSequenceNumber=$linkSequenceNumber, previousHash=${previousHash.contentToString()}, signature=${signature.contentToString()}, timestamp=$timestamp, insertTime=$insertTime, blockId='$blockId', linkedBlockId='$linkedBlockId', isGenesis=$isGenesis, isSelfSigned=$isSelfSigned, isProposal=$isProposal, isAgreement=$isAgreement, transaction=$transaction, hashNumber=$hashNumber)"
    }
